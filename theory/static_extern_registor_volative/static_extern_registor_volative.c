...
VOLATIVE : từ khóa dành cho những biến hoạt động trong các sự kiện // với hàm main
hàm main không trực tiếp thay đổi giá trị
...
int volatile foo;//... and this way are OK! Define a volatile integer variable

volatile uint8_t *pReg;//both this way...
uint8_t volatile *pReg;//... and this way are OK! Define a pointer to a volatile unsigned 8-bit integer
...
RAM
ring buffer
alias to memory(vay mượn bộ nhớ)(remap bộ nhớ)
ofset để đọc đúng vùng nhớ ánh xạ
...
int main() {
    printf("%d", 1);
    static volatile unsigned long long int a;
